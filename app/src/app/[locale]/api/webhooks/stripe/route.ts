        ...
        case "payment_intent.succeeded": {
            ...
            if(user)
            {
                ...
                user.payments.push({
                    value: coins,
                    coins,
                    status: PaymentDone.CONFIRMED
                })
                await user.save()
                return new Response(
                    JSON.stringify({ message: "Provided with success" }),
                    {
                        status: 200,
                    }
                );
                ...
                return new Response(JSON.stringify({ error: "No user in DB" }), {
                    status: 403,
                });
            }
        }
        ...
        case "payment_intent.payment_failed": {
            ...
            if(user)
            {
                user.payments.push({
                    value: coins,
                    coins,
                    status: PaymentDone.ERROR
                })
                await user.save()
                return new Response(
                    JSON.stringify({ message: "Provided with error" }),
                    {
                        status: 200,
                    }
                );
